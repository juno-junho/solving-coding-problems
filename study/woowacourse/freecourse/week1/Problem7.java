...
public class Problem7 {
    // 팀에 속한 크루 중 평소 알고리즘에 관심이 많은 미스터코는 친구 추천 알고리즘 구현
    // 사용자와 함께 아는 친구 수 : 10점 // 사용자의 타임 라인에 방문한 횟수 1점
    // 사용자 아이디 user와 친구 관계 friends, 타임라인 방문기록 visitors 주어짐
    // 미스터 코의 친구 추천 규칙에 따라 점수 가장 높은 순으로 정렬해 최대 5명 return 하는 solution 메소드.
    // 추천 점수가 0점인 경우 추천 x, 추천 점수가 같은 경우에는 이름순으로 정렬
    ...
    public static List<String> solution(String user, List<List<String>> friends, List<String> visitors) {
        ...
        // hashmap에 친구리스트의 친구를 저장하면서 value값 올리기.
        ...
        // *********** 0인 경우도 제외 시켜야함!! distinct() ???
        return recommendedUser.stream().sorted().limit(5).map(User::getId).collect(Collectors.toList());
//        List<User> result = new ArrayList<>(recommendedUser);
//        Collections.sort(result);
//        result.forEach(i -> System.out.println("i.getId() = " + i.getId() + "point : " + i.getPoint()));
//        return result.stream().limit(5).map(i -> i.getId()).collect(Collectors.toList());
    }
...
class ListGraph {
    ...
    public User getUser(User userA) {
        if (!listGraph.contains(userA)) return null;
        for (User user : listGraph) {
            if (user.equals(userA)) return user;
        }
        return null;
    }
    public void put(User userA, User userB) {
        // userA와 userB를 set에서 찾기
        // listgraph에 user가 없다면 생성하기
